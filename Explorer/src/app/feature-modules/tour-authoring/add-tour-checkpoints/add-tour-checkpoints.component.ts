...
import { MiscEncounterDto } from '../../encounters/model/encounter.model';
...
export class AddTourCheckpointsComponent implements OnInit {
    ...
    addMiscEncounter(): void {
        if(this.checkpointForm.value.actionDescription && this.checkpointForm.value.description) {
        var encounter: MiscEncounterDto = {
            id:0,
            name: this.checkpointForm.value.name + "Encounter" || "Checkpoint Encounter",
            description: this.checkpointForm.value.description,
            lattitude: Number(this.checkpointForm.value.latitude || 0),
            longitude: Number(this.checkpointForm.value.longitude || 0),
            actionDescription: this.checkpointForm.value.actionDescription
        }
        this.encounterService.createEncounter(encounter).subscribe({
            next: (createdEncounter) => {
            },
            error: (err) => {
                console.error("Error creating misc encounter:", err);
            }
        });
    }
    }
